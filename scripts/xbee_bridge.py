...
from ros2_xbee_bridge.serialization import deserialize_message, serialize_message
...
class RelayNode(Node):
    def __init__(self):
        ...
        self.config_dir = pathlib.Path(self.get_parameter("config_dir").get_parameter_value().string_value)
        self.declare_parameter(
            "xbee_device_list_file", descriptor=ParameterDescriptor(type=Parameter.Type.STRING.value)
        )
        ...
        self.declare_parameter(
            "incoming_message_prefix", descriptor=ParameterDescriptor(type=Parameter.Type.STRING.value)
        )
        self.recv_message_prefix = self.get_parameter("incoming_message_prefix").get_parameter_value().string_value
        self.declare_parameter(
            "monitoring_station_name", descriptor=ParameterDescriptor(type=Parameter.Type.STRING.value)
        )
        self.monitoring_station_name = self.get_parameter("monitoring_station_name").get_parameter_value().string_value
        self.declare_parameter("compression_level", descriptor=ParameterDescriptor(type=Parameter.Type.INTEGER.value))
        self.compression_level = self.get_parameter("compression_level").get_parameter_value().integer_value
        self.declare_parameter(
            "max_transmission_retries", descriptor=ParameterDescriptor(type=Parameter.Type.INTEGER.value)
        )
        ...
        self.ready_srv = self.create_service(
            Empty, f"{self.get_name()}/ready", self.empty_callback, callback_group=default_cb_group
        )
    ...
    def check_xbee(self):
        ...
        This method attempts to get the power level of the XBee device. If the
        device is disconnected, it logs a fatal error message and exits the
        program. ROS launch will attempt to restart the node in the hope that
    ...
    def preload_topics(self):
        """
        Preload topics from a configuration file and create bridges for each
        topic.

        The configuration file path is obtained from the
        'ros1_bridge_topics_file' parameter. If the parameter is not set or the
        file cannot be opened, the method returns without creating any bridges.
        """
        self.declare_parameter(
            "ros1_bridge_topics_file", descriptor=ParameterDescriptor(type=Parameter.Type.STRING.value)
        )
    ...
    def check_new_topics(self):
        ...
        This method compares the current topics in the ROS system with the known
        topics, and creates bridges for any new topics that are detected. It
        updates the set of known topics with the newly detected topics.
        """
        ...
        if new_topics:
            ...
            new_topics_and_types = [
                {"topic": topic, "type": current_topics_and_types[topic][0]} for topic in new_topics
            ]
    ...
    def create_bridges(self, topics_and_types):
        """
        Creates bridges between ROS 2 topics and XBee destinations based on the
        given topics and types.

        Args:
            topics_and_types (list): A list of dictionaries containing topic and
                                     type information.
        """
    ...
    def create_bridge_ros2xbee(self, topic, msg_description, msg_type):
        ...
        Args:
            ...
            msg_description (str): The description of the message.
                                   (needed for rosbridge_suite)
            msg_type (Any): The type of the message as a message object.
                            (needed for ros2)
        ...
        if self.ros_version == "ros1":
            ...
            bridge = self.create_subscription(
                msg_type, topic, (lambda x, t=topic: self.xbee_tx_cb(x, t)), 1, callback_group=self.subs_cb_group
            )
        else:
    ...
    def xbee_rx_cb(self, msg: XBeeMessage):
        ...
        self.get_logger().info(
            f"Received message: {'(incomplete)' if in_msg is None else ''}\n"
        ...
        for topic in topics:
            if topic in self.pubs:
                ...
                self.get_logger().warn("Publisher for this topic is not initialized.\n")

    def send_timer_cb(self):
        """
        Callback function for sending messages from the message queue.

        This function retrieves a message from the message queue and sends it to
        the appropriate destination. If the message fails to transmit or times
        out, it may be retransmitted with a decreased priority. Messages sent to
        the monitoring station are not queued for retransmission.
        """
        try:
            priority, tx_counter, data, dest = self.msg_queue.get_nowait()
            self.get_logger().info(
                f"Publishing message from queue:\n" f"\tmessage id: {tx_counter}\n" f"\tlength: {len(data)} bytes"
            )
            retransmit = False
            ...
            if retransmit:
                ...
                if new_priority > self.max_retries:
                    ...
                    self.get_logger().info(
                        f"Re-adding new message to queue>\n"
                        f"\tmessage id: {tx_counter}\n"
    ...
    def deserialize(self, msg, sender):
        ...
        Returns:
            Tuple[List[str], Any]: A tuple containing a list of topics and the
        ...
        if self.compression_level != 0:
            try:
                ...
                self.get_logger().warn("Zlib decompression failed. Trying raw message.")
                decompressed = data
        else:
            decompressed = data
        ...
        if len(topics) == 2 and self.msg_types[topics[0]] != self.msg_types[topics[1]]:
            self.get_logger().warn(
                f"There are subscribers for *both* unicast and broadcast of topic {topic}, but they"
                f"have different types {self.msg_types[topics[0]]} != {self.msg_types[topics[1]]}. "
                f"Publishing only to unicast subscribers."
            )
            # The logic behind publishing only to unicast is that they are probably
            # subscribing with a more specific use case.

        expected_data_len = decompressed[topic_len + 1]
        real_data_len = len(decompressed[topic_len + 2 :])
        if real_data_len != expected_data_len:
            self.get_logger().warn(
                f"Message length mismatch." f"Expected {expected_data_len} bytes, got {real_data_len} bytes."
            )
